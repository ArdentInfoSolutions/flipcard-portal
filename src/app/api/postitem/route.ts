...
export async function GET() {
  try {
    ...
    // Convert links_or_images back to objects if needed
...
export async function POST(req: NextRequest) {
  try {
    ...
    // ✅ Fix: Query user by `id` not `user_id`
    ...
    if (postType === "web") {
      content = webLinks;
    } else if (postType === "videos" && Array.isArray(videos)) {
    ...
    const result = await query(
      `INSERT INTO postitem (title, promo, description, post_type, categories, links_or_images, user_id)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING *`,
      [title, promo, description, postType, categories, JSON.stringify(content), String(userId)]
    );
